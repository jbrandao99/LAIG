...
class XMLscene extends CGFscene {
  ...
  update(currentTime) {
    if (this.initialTime == 0) {
      ...
      for (var i = 0; i < this.graph.components.length; i++) {
        if (this.graph.components[i].animation.animationref != null)
          this.graph.components[i].animation.animationref.update(
              elapsedTime / 1000);
      }
    }
  }
  ...
  initCameras() {
    this.camera = new CGFcamera(
        0.8, 0.1, 500, vec3.fromValues(15, 15, 15), vec3.fromValues(0, 0, 0));
  }
  ...
  initViews() {
    ...
    for (var key in this.graph.views) {
      ...
      if (this.graph.views[key].type == 'perspective') {
        ...
        this.views[key] = new CGFcamera(
            angle, near, far, vec3.fromValues(from[0], from[1], from[2]),
            vec3.fromValues(to[0], to[1], to[2]));
      } else if (this.graph.views[key].type == 'ortho') {
        ...
        var up = this.graph.views[key].up;

        this.views[key] = new CGFcameraOrtho(
  ...
  onGraphLoaded() {
    ...
    this.interface.addLightsFolder(this.graph.lights);
  ...
  display() {
    ...
    if (this.sceneInited) {
      ...
      if (this.graph.lengthSandLengthTset == false) {
        this.graph.setLengthSandLengthT();
        this.graph.lengthSandLengthTset = true;
      }

      if (this.gui.isKeyPressed('KeyM')) this.graph.changeMaterials();
    }
